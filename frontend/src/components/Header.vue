...
<template>
          ...
          <div class="chooseModelBox">
              ...
              >
                  <template #reference>
                      ...
                      <div v-if="userModel=='3.5'" class="typeTab baseType">基础版</div>
                      <div v-if="userModel=='4'" class="typeTab plusType">Plus版</div>
                  </template>
                  <template #default>
                    <div v-if="loginStatus && screenWidth<=900" class="account"><div class="userPicture"></div> {{ account }}</div>
                        ...
                        <div class="plusBox" @click="changeMosel('4')" :class="userModel=='4'?'acivePlus':''">
                          ...
                          <template v-if="accountInfo.user_account">
                            <div class="timestime" v-if="accountInfo.user_account.terminable_time">
                              ...
                              <div class="time">到期：{{ accountInfo.user_account.due_date.split(" ")[0] }}</div>
                            </div>
                          ...
                          <div class="timestime" v-if="!accountInfo.user_account?.terminable_time && !accountInfo.user_account?.un_terminable_time ">
                            <el-tooltip
                              class="box-item"
                              effect="dark"
                              content="PLUS模型每日限时免费2次"
                              placement="top-start"
                            >
                              <div v-if="!loginStatus" class="xianmian">限免2条</div>
                              <div v-else class="xianmian">限免{{accountInfo.allowance_num}}条</div>
                            </el-tooltip>
                           
                            <div class="time">到期：{{ expirationTime }}</div>
                          </div>
                          <div v-if="userModel=='4'"  class="bottomBtn" @click="goSubscribe">{{ accountInfo?.user_account?'续费':'订阅' }}</div>
                        </div>
                        <div class="signout-mb" v-if="loginStatus && screenWidth<=900" @click="logout">退出</div>
                      </div>
                  </template>
              </el-popover>
          </div>
  ...
  <SubscribeType 
    ...
    :isPlus="accountInfo.user_account"
    :paytypelist="paylist"
  ...
  setup() {
    ...
    const changeMosel = (model)=>{
      if(model=='4'){
        ...
        if(!accountInfo.value.user_account && !accountInfo.value.allowance_num && !accountInfo.value.user_account?.terminable_time && !accountInfo.value.user_account?.un_terminable_time){
          goSubscribe()
          return
        }
      }
    ...
    watch(() => userPopover.value, async(val,old) => {
